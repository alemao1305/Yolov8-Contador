...
def gera_qr_code():
    ...
    if not minutos:
        messagebox.showinfo(title="Valor não digitado", message="Caso o valor seja digita, o programa por padrão ira adicinar 5 minutos como intervalo de contagem")
...
def create_tkinter_interface():
    global window, website_entry, website_entry2
    window = Tk()
    ...
    Label(window, text= "Minutos").grid(row=3, column=0)
    website_entry2 = Entry(window, width=4)
    website_entry2.grid(row=3,column=1, columnspan=2)
...
# Definir o valor padrão para 5 minutos caso 'minutos' não seja recebido
if not minutos:
    minutos = 5
else:
    try:
        ...
        raise ValueError("O valor de minutos deve ser um número inteiro ou decimal.")
# Agendar a função para ser executada a cada 5 minutos
schedule.every(minutos).minutes.do(agendar_envio_dados)
